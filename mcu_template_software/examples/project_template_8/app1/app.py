...
class ThisMachine(BaseMachine):
    ...
    def start(self):
        ...
        # Call the ntp task to periodically synchronize the local MCU time.
    ...
    async def ntp_task(self):
        # If we have not yet created an NTP instance create one.
        if self._ntp is None:
            # Attempt to update the MCU time via NTP every 2 hours.
        ...
        while True:
            ...
            if not self._ntp_sync_success_once:
                ...
                self._saved_up_time = time() - self._startTime
            ...
            if not self._ntp_sync_success_once and ntp_sync_success:
                ...
                self._startTime = time() - self._saved_up_time
                ...
                self._web_server.set_start_time(self._startTime)
    ...
    async def app_task(self):
        ...
        while True:
            ...
            lt_str = ""
            if self._ntp:
                lt_str = self._ntp.get_local_time_string()
            print(f"{lt_str} app_task(): count = {count}")
            await asyncio.sleep(1)
            count += 1
